...
```hcl
module "sqs" {
    source = "github.com/pbs/terraform-sqs-module?ref=x.y.z"
}
```
...
gh -R 'pbs/terraform-sqs-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/sqs'
rm -f release.tar.gz
```
...
```bash
git remote add -f sqs git@github.com:pbs/terraform-sqs-module.git
git subtree add --prefix terraform/modules/sqs sqs main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/sqs sqs $REF --squash
```
